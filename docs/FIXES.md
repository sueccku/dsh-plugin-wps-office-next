...
### 50. P5 收尾：逃生舱去留、广告面与预算重定、文档重生成

**P5-1 `execute_method` 的决策：保留，但保持隐藏，并把契约写死。**

证据：未工具化 action 台账已经降到 **7 个刻意的重复实现**，日常根本不需要它；两次真实 e2e
（两场景、96 次工具调用）与 541 项测试里**没有任何一次用过它**。但它仍有一个不可替代的用途：
水印与文档属性是**实测的 WPS 缺口**（见 FIXES 45），没有它就没有任何出路。
所以决定是保留为隐藏逃生舱，并把描述从「执行自定义WPS API方法」改成明确的最后手段契约
（先 `wps_help`／技能参考表找现成工具，确认没有才用它，参数与返回值都不校验）。

**P5-2 广告面与预算重定（D1）**：P4 结束时广告面 66 / 35,568 字节，D1 上限 70 / 40,000，
还剩 4 个工具位置。用掉 3 个给「隐藏但高频」的能力：`wps_word_get_comments`（读批注）、
`wps_word_set_table_cell`（写表格单元格，与已广告的 `get_tables` 成对）、
`wps_ppt_set_table_format`（表格外观）。

- 最终广告面：**69 工具 / 37,573 字节**（上限 70 / 40,000；余量 1 个工具 / 2,427 字节）
- **上限本身不再调整**：D1 的 70 / 40,000 当初就是为 P2–P4 这个范围定的，最终落点几乎正好压在线上，
  说明这个数字定得合理；再收紧反而会挡住后续正常维护
- `wps_help` 紧凑索引与 4 份技能 `reference.md` 已按最终注册表重新生成（含新合并的三个工具）

**P5-3 文档同步**：README 的三档数字、工具面构成（Excel 118 / Word 59 / PPT 76 / 通用 14）、
测试表、FIXES/PROGRESS 全部对齐到最终状态；本文即最终数字的落点。

**最终数字（P5 收尾时）**：

| 项 | 值 |
|---|---|
| 注册工具 | **267**（Excel 118 / Word 59 / PPT 76 / 通用 14） |
| 广告面 | **69 工具 / 37,573 字节**（minimal 4 / 1,348；full 267 / 153,777） |
| 桥 action | **267** |
| 测试 | **541 项 / 25 文件**（真实 WPS） |
| 门禁 | verify 23、spec 复现 12、参数契约 255 对（A/B/C/D 全 0）、e2e 28 项 |
| 台账 | 别名债务 59、未工具化 action 7（全部刻意） |
