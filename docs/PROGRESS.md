...
- **第一批 5 个 Excel 动作**（clearRange / clearFormats / deleteRows / deleteColumns / deleteSheet）在动手前回传
  `data.impact`：`{ address, cells, nonEmpty, preview }`；桥头两个只读助手三级回退取 `nonEmpty`（`WorksheetFunction.CountA` → `Application.CountA` → 小范围手工扫描），
  `preview` 只在 ≤ 4096 格时读 `Value2`；TS 侧 `impact.ts` 压成一句中文给模型复述
- **实测**：`test/destructive-guard.test.mjs` **16 项全绿**；整列 1048576 格 / 整行 16384 格只回 `cells`+`nonEmpty`（性能取舍）；
  `spec` / `skills` 重新生成后无漂移；记录见 `docs/FIXES.md` 53
- **剩余**：其它 24 个动作的统计，以及各动作确认框的逐项实测（清单「待办」列）
- 全套 **611 项 / 29 文件**；verify 23、spec 复现 12、参数契约 255 对（A/B/C/D 全 0）
...
| 桥 action | — | **267** |
| 测试 | 0 | **611 项 / 29 文件**（其中 4 个文件不需要 WPS，已进 CI） |
...
**611/0**、一键 e2e **28/28** 全绿，可以开始小范围推广；第 2～4 波（S3 数据安全 / S4 冒烟矩阵 /
