...
- 全套 **641 项 / 29 文件**；verify 23、spec 复现 13、参数契约 255 对（A/B/C/D 全 0）

## 加固第 2 波：S4 逐工具覆盖率账本（第一片已落地）

- **先量覆盖率**：267 个注册工具里 **161 个**被测试或 e2e 点名（Excel 87/118、Word 41/59、PPT **25/76**）
- `scripts/smoke-tools.mjs`：静态矩阵（按应用分组 + 未覆盖清单）+ `--check` ratchet（161）+ `--live`
...
- **剩余**：PPT 的 50 个未覆盖工具按使用场景补测试；记录见 `docs/FIXES.md` 56
- 全套 **641 项 / 29 文件**
...
| 桥 action | — | **267** |
| 测试 | 0 | **641 项 / 29 文件**（其中 4 个文件不需要 WPS，已进 CI） |
| 门禁 | 无 | verify 23、spec 复现 13、参数契约 255 对（A/B/C/D 全 0）、一键 e2e 28 项 |
...
**641/0**、一键 e2e **28/28** 全绿，可以开始小范围推广；第 2～4 波（S3 数据安全 / S4 冒烟矩阵 /
