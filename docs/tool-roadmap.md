...
| ↳ P1 进展 | 前两步已落地（FIXES 33/34） | spec 209 条 + 生成器 + 复现验收 9 项全绿；调用点解析器已抽成 `scripts/lib/tool-action-map.mjs`（param-contract 与提取器共用），重命名债务 57 → **27**（棘轮收紧）。剩余债务：同义改名 27、未工具化 action 29、raw schema 32、别名 29、容器 12 | 下一步 **P1-4**：把 27 处同义改名与 12 个容器收干净，让公开名 = 桥键，三张人手表归零；然后 P1-5 让 param-contract 退化为相等断言 | — |
