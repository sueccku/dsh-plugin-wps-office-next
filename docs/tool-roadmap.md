...
| ↳ P1 进展 | 前四步已落地（FIXES 33–36） | spec 209 条 + 生成器 + 验收 **12 项全绿**；调用点解析器共享；每个参数都有明确去向（未归类 0）；**别名表与容器表已从生成器搬进 spec**（`mcp/src/spec/aliases.ts` → 生成的 JSON），宿主产物**逐字节未变**。剩余债务：别名 62（改名的两种机制已分清）、未工具化 action 29、raw schema 32 | 下一步 **P1-5**：键表也改由 spec 产出（`$helperKeys` 随之消失），param-contract 退化为相等断言；然后进 P2 | — |
