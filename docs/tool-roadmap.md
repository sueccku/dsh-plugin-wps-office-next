...
| ↳ P1 进展 | 第一步已落地（FIXES 33） | spec 209 条 + 生成器 + 复现验收 9 项全绿：209 schema 与序列化字节数完全复现；债务入账（重命名 58、未工具化 action 29、raw schema 32、别名 15、容器 12） | 下一步：桥键真源（复用 param-contract 的调用点解析器）+ P1-4 三表归零 | — |
