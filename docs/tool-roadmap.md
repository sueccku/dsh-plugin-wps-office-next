...
| ↳ P1 进展 | 前三步已落地（FIXES 33/34/35） | spec 209 条 + 生成器 + 验收 **11 项全绿**；调用点解析器共享（`scripts/lib/tool-action-map.mjs`）；**每个参数都有明确去向**（bridge 509 / local 22 / 折进单参数 3，未归类 0）——「每个 bridge 参数都落在桥真正读的键上」已是严格断言。剩余债务：别名 62、未工具化 action 29、raw schema 32、容器 12 | 下一步 **P1-4b**：把 62 处别名（公开名 ≠ 桥键）对齐、12 个容器形态收进 spec 生成，然后 P1-5 让 param-contract 退化为相等断言 | — |
| **P2 Excel 做深** | 一站式表格 | 5 个波次（§5.3） | Excel 对象不变量全绿 + 覆盖报告 + Excel 重场景 e2e | P1 |
| **P3 Word 做深** | 一站式文档 | 挂已有 4 项 + 表格读写 + 文档生产族 | 同上 | P1 |
...
| P2-1 | 挂出已有能力（零 COM 成本） | 21 个 Excel 缺口全部挂工具：named range get/delete、条件格式 get/remove、数据验证 get/remove、autofit×3、wrapText、groupColumns、unfreezePanes、findInSheet/replaceInSheet、copyFormat/clearFormats、refreshLinks、consolidate、calculateSheet、getExcelContext、getActiveWorkbook、openFile；**used range / sheet info 升为一等公民**（范围地址、行列数、表头、可见性、标签色） |
| P2-2 | ListObject 全族（新桥代码） | 从区域建表、列表/读取结构、增删行、总计行、表格样式、转回区域、resize、结构化引用、表内自动筛选 |
...
| ID | 波次 | 内容 |
|---|---|---|
| P3-1 | 挂出已有能力 | `getBookmarks`、`getComments`、`getDocumentStats`、`insertHyperlink`、域列表、修订查询 |
| P3-2 | 表格读写编辑（新桥代码） | 读取表格结构、单元格读写、增删行列、合并/拆分、表格样式、表格 ↔ 文本转换 |
| P3-3 | 文档生产 | 页码、水印、文档属性、分栏、页眉页脚进阶、目录更新、修订接受/拒绝、批注读删 |
| P3-4 | 长尾（按 D2 档位） | 内容控件、脚注/尾注、邮件合并、索引/交叉引用 |
...
| P5-2 | 按 P2 结果重定广告面与预算（D1），重生成 `wps_help` 紧凑索引 |
