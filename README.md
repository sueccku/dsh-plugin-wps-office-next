...
当前数字：**640 项测试（29 个文件，多数需要真实 WPS）+ verify 23 项 + spec 复现 12 项**全绿；
...
`verify --static`、参数契约对账、以及四个不碰真实 WPS 的测试文件
（`plugin` / `com-host` / `host-lease` / `watchdog`）。需要真实 WPS 的测试与一键 e2e 留在本机。
