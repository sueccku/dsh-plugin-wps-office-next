...
一个包覆盖 WPS 三大应用，共 **237 个 MCP 工具**（每次请求只广告其中 44 个，其余按需查询后调用）：
...
| P3 工具面收敛（250 → 44 个广告工具） | 完成，预算上限 45 工具 / 25,000 字节 |
| P4 缺陷修复（工具面/参数契约/静默失败） | 完成，29 条修复记入 [docs/FIXES.md](docs/FIXES.md) |
| P5 技能文档（路由 + 三个应用 + 生成的参考表） | 完成，4 个技能 310 项测试覆盖 |
| 真实端到端验证 | 完成两轮，见下 |
...
**7 秒**返回 `connected: true、advertisedTools: 44、registeredTools: 237、hiddenTools: 193`，
...
| full | 237 | 133,097 | 38,028 | 全量，保留完整描述 |

注册目录 237 个工具，其中：广告 44、隐藏 193（含 18 个弃用转发名与 16 个上游遗留 builtin）；
按应用分布为 Excel 75 / Word 32 / PPT 87 / 通用 9 / builtin+门面 16。
...
| 默认工具面 | standard 档 44 个工具；预算上限 45 工具 / 25,000 字节 |
