...
| P5 技能文档（路由 + 三个应用 + 生成的参考表） | 完成，4 个技能 370 项测试覆盖 |
| 真实端到端验证 | 完成两轮，见下 |
...
- 370 项测试 + 23 项门禁全绿，参数契约对账 215 对、四类静默失效均为 0；
...
当前数字：**370 项测试**（18 个文件，含 spec 复现验收 12 项）+ **23 项门禁**全绿；广告面
**51 工具 / 26,652 字节**（D1 上限 60 / 32,000）；桥 action **231**（生成器断言源码、生成物、期望值三方一致）；
注册工具 **227**，全部进操作规格（spec）。
...
| test/ | 18 个回归测试文件、370 项断言（多数需要本机 WPS） |
