...
| P5 技能文档（路由 + 三个应用 + 生成的参考表） | 完成，4 个技能 509 项测试覆盖 |
| 真实端到端验证 | 完成两轮，见下 |
...
- 509 项测试 + 23 项门禁全绿，参数契约对账 267 对、四类静默失效均为 0；
...
| full | 279 | 160,658 | 45,902 | 全量，保留完整描述 |

注册目录 **279** 个工具：广告 66、隐藏 213；另有 18 个已合并的旧名字**不占注册位**，只在派发期解析成规范工具
（旧名照样能调，`wps_help {tool:"旧名"}` 会告诉你该用哪个）。按应用分布为 Excel 118 / Word 59 / PPT 88 / 通用 9 / builtin+门面 5。
...
当前数字：**509 项测试**（24 个文件，含 spec 复现验收 12 项）+ **23 项门禁**全绿；广告面
**66 工具 / 35,568 字节**（D1 上限 70 / 40,000）；桥 action **279**（生成器断言源码、生成物、期望值三方一致）；
注册工具 **279**，全部进操作规格（spec）。
...
| test/ | 24 个回归测试文件、509 项断言（多数需要本机 WPS） |
