...
安装也简单：把下面「怎么装」那一段整段复制给你的 DSH AI，剩下交给它。你唯一要亲手做的是**重启一次 DSH**。
...
完整的工具清单在 [skills/](skills/) 各目录下的 reference.md 里；在 DSH 里直接问「WPS 能做哪些事」也行。
...
AI 装完后，按它说的**完全关闭 DSH 再重新打开**。重启后新建一个会话，问一句「WPS 连上了吗」就行。
...
逐项检查系统位数、Node 版本、PowerShell STA、包内容是否完整、WPS 能否连上、**已安装 WPS 的版本与架构**（≥ 12.1 且 64 位）以及**插件接线**（`cordis.patch.yml` 里是否有 `wps-office-next-plugin` 与 `mcp-wps-office-next` 两个 id），最后打印 `DOCTOR OK` 或具体错误。
...
<details>
<summary><b>进程与状态文件</b></summary>

插件在本机留下的东西只有两处，都在你的用户目录下，删掉不影响 WPS：

- `~/.wps-office-mcp/com-host.json`：宿主的租约与心跳（谁在占用 WPS、忙不忙）。同一时间只允许一个会话操作 WPS，靠它判断上一个宿主是不是真的死了。
- `~/.wps-office-mcp/owned-apps.json`：本会话**自己启动**的 WPS 实例清单。客户端被强杀时（Windows 会把整棵进程树一起结束），下一个宿主凭这份清单把孤儿实例收掉；有未保存内容的实例一律不动。
...
二者均为 MIT 许可，来源与处理方式见 [THIRD_PARTY_NOTICES.md](THIRD_PARTY_NOTICES.md)。经过多轮重构与扩展，本项目的架构、工具面与代码已与上游有很大差异，是独立演进的结果。
