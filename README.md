...
| full | 267 | 153,600 | 43,886 | 全量，保留完整描述 |

注册目录 **267** 个工具：广告 66、隐藏 201；另有 18 个已合并的旧名字**不占注册位**，只在派发期解析成规范工具
（旧名照样能调，`wps_help {tool:"旧名"}` 会告诉你该用哪个）。按应用分布为 Excel 118 / Word 59 / PPT 76 / 通用 9 / builtin+门面 5。
...
**66 工具 / 35,568 字节**（D1 上限 70 / 40,000）；桥 action **267**（生成器断言源码、生成物、期望值三方一致）；
