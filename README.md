...
| P5 技能文档（路由 + 三个应用 + 生成的参考表） | 完成，4 个技能 340 项测试覆盖 |
| 真实端到端验证 | 完成两轮，见下 |
...
- 340 项测试 + 23 项门禁全绿，参数契约对账 205 对、四类静默失效均为 0；
...
| **standard（默认）** | **44** | **23,593** | **6,741** | 门面 + 40 个精选工具，字节数较基线降 83.4% |
| full | 209 | 121,308 | 34,659 | 全量，保留完整描述 |

注册目录 **209** 个工具：广告 44、隐藏 165；另有 18 个已合并的旧名字**不占注册位**，只在派发期解析成规范工具
（旧名照样能调，`wps_help {tool:"旧名"}` 会告诉你该用哪个）。按应用分布为 Excel 75 / Word 32 / PPT 88 / 通用 9 / builtin+门面 5。
...
当前数字：**340 项测试**（17 个文件，含 spec 复现验收 12 项）+ **23 项门禁**全绿；广告面
**48 工具 / 25,097 字节**（D1 上限 60 / 32,000）；桥 action **231**（生成器断言源码、生成物、期望值三方一致）；
注册工具 **217**，全部进操作规格（spec）。
...
| test/ | 17 个回归测试文件、340 项断言（多数需要本机 WPS） |
