...
| P3 工具面收敛（250 → 44 个广告工具） | 完成；P2 做深后广告面 48，预算按 D1 提到 60 工具 / 32,000 字节 |
| P4 缺陷修复（工具面/参数契约/静默失败） | 完成，29 条修复记入 [docs/FIXES.md](docs/FIXES.md) |
| P5 技能文档（路由 + 三个应用 + 生成的参考表） | 完成，4 个技能 419 项测试覆盖 |
| 真实端到端验证 | 完成两轮，见下 |
...
- 419 项测试 + 23 项门禁全绿，参数契约对账 231 对、四类静默失效均为 0；
...
| **standard（默认）** | **56** | **30,885** | **8,824** | 门面 + 52 个精选工具，字节数较基线降 78.2% |
| full | 243 | 143,051 | 40,872 | 全量，保留完整描述 |

注册目录 **243** 个工具：广告 56、隐藏 187；另有 18 个已合并的旧名字**不占注册位**，只在派发期解析成规范工具
（旧名照样能调，`wps_help {tool:"旧名"}` 会告诉你该用哪个）。按应用分布为 Excel 109 / Word 32 / PPT 88 / 通用 9 / builtin+门面 5。
...
当前数字：**419 项测试**（20 个文件，含 spec 复现验收 12 项）+ **23 项门禁**全绿；广告面
**56 工具 / 30,885 字节**（D1 上限 60 / 32,000，只剩 1,115 字节余量）；桥 action **247**（生成器断言源码、生成物、期望值三方一致）；
注册工具 **243**，全部进操作规格（spec）。
...
| test/ | 20 个回归测试文件、419 项断言（多数需要本机 WPS） |
...
| 默认工具面 | standard 档 48 个工具；预算上限 60 工具 / 32,000 字节（D1） |
