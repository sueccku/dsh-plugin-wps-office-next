...
$script:ActionParamKeys = @{
    ...
    'addContentControl' = @('tag', 'text', 'title', 'type')
    ...
    'setPptTableFormat' = @('backgroundColor', 'bold', 'col', 'fontColor', 'fontSize', 'height', 'left', 'presentationName', 'row', 'slideIndex', 'tableIndex', 'tableName', 'top', 'width')
    ...
    'setSlideFooter' = @('autoUpdate', 'dateFormat', 'footerText', 'presentationName', 'showDate', 'showFooter', 'showSlideNumber')
...
# ==================== Excel 表（ListObject）====================
    ...
    "createListObject" {
        ...
        if ($null -eq $excel) { Output-Json @{ success = $false; error = "WPS Excel not running" }; return }
        $sheet = Get-WorksheetByParam $excel $p
        if (-not $p.range) { Output-Json @{ success = $false; error = "range required" }; return }
    ...
    "addListRow" {
        ...
        if ($null -eq $lo) { Output-Json @{ success = $false; error = "table not found on this sheet" }; return }
        try { $null = $lo.ListRows.Add() } catch { Output-Json @{ success = $false; error = $_.Exception.Message }; return }
        if ($null -ne $p.values) {
            $vals = @($p.values)
            if ($vals.Count -gt 0) {
    ...
    "deleteListRow" {
        ...
        $idx = [int]$p.rowIndex
        if ($idx -lt 1 -or $idx -gt $total) { Output-Json @{ success = $false; error = ("rowIndex " + $idx + " out of range 1.." + $total) }; return }
        $name = [string]$lo.Name
        try { $lo.ListRows.Item($idx).Delete() } catch { Output-Json @{ success = $false; error = $_.Exception.Message }; return }
    ...
    "setListObjectTotals" {
        ...
        try {
            ...
            if ($p.column) {
                ...
                if ($null -eq $col) { Output-Json @{ success = $false; error = "column not found in this table" }; return }
    ...
    "resizeListObject" {
        ...
        if ($null -eq $lo) { Output-Json @{ success = $false; error = "table not found on this sheet" }; return }
        if (-not $p.range) { Output-Json @{ success = $false; error = "range required" }; return }
        try { $null = $lo.Resize($sheet.Range([string]$p.range)) } catch { Output-Json @{ success = $false; error = $_.Exception.Message }; return }
        Output-Json @{ success = $true; data = (Get-ListObjectInfo $lo $sheet.Name ([string]$p.range)) }
    }
    ...
    "unlistListObject" {
        ...
        try { $lo.Unlist() } catch { Output-Json @{ success = $false; error = $_.Exception.Message }; return }
        Output-Json @{ success = $true; data = @{
...
# ==================== Word 长尾（P3-4，新 COM 代码）====================
    ...
    "addContentControl" {
        ...
        if ($null -eq $ccType) { Output-Json @{ success = $false; error = ("unknown content control type: " + $p.type) }; return }
    ...
    "addFootnote" {
        ...
        $range = Get-MainTextRange $word $doc
        try { $doc.Footnotes.Add($range, [string]$p.text) | Out-Null } catch { Output-Json @{ success = $false; error = $_.Exception.Message }; return }
    ...
    "addEndnote" {
        ...
        if (-not $p.text) { Output-Json @{ success = $false; error = "text required" }; return }
        $range = Get-MainTextRange $word $doc
        try { $doc.Endnotes.Add($range, [string]$p.text) | Out-Null } catch { Output-Json @{ success = $false; error = $_.Exception.Message }; return }
    ...
    "insertCrossReference" {
        ...
        if ($null -eq $p.referenceItem) { Output-Json @{ success = $false; error = "referenceItem required" }; return }
    ...
    "mailMerge" {
        ...
        if ($null -eq $path) { Output-Json @{ success = $false; error = ("data file not found: " + $p.dataFile) }; return }
        $fields = @()
...
# P4-2：把 setShapeShadow / setShapeGradient / setShapeBorder / setShapeTransparency 四个碎片 setter
    ...
    "setShapeEffect" {
        ...
        if ($p.borderStyle) {
            ...
            if ($null -eq $dash) { Output-Json @{ success = $false; error = ("unknown borderStyle '" + $p.borderStyle + "'; use solid/dash/dot/dash_dot/dash_dot_dot") }; return }
        ...
        if ($applied.Count -eq 0) { Output-Json @{ success = $false; error = "nothing to apply: give at least one effect property" }; return }
...
# P4 第二波：三组合并（FIXES 49）。
    ...
    "setAnimation" {
        ...
        if ($p.preset) {
            ...
            if ($null -eq $config) { Output-Json @{ success = $false; error = ("unknown preset '" + $p.preset + "'; use fadeIn/flyIn/zoomIn/wipeIn/appear") }; return }
            $delay = 0
...
return }
        if ($null -eq $p.shapeName -and $null -eq $p.shapeIndex) { Output-Json @{ success = $false; error = "shapeIndex/shapeName is required (or give preset to animate the whole slide)" }; return }
...
return }
    ...
    "setPptTableFormat" {
        ...
        if ($applied.Count -eq 0) { Output-Json @{ success = $false; error = "nothing to apply: give geometry or cell style properties" }; return }
    ...
    "setSlideFooter" {
        ...
        if ($null -ne $p.dateFormat -and "$($p.dateFormat)" -ne "") {
            ...
            try {
                ...
                Output-Json @{ success = $false; error = ("unsupported date format '" + "$($p.dateFormat)" + "': " + $_.Exception.Message) }; return }
        }
        if ($useFixed -eq $true) { $dateTime.UseFormat = $false; $dateTime.Text = "" }
        if ($applied.Count -eq 0) { Output-Json @{ success = $false; error = "nothing to apply: give footer/date/slide-number properties" }; return }
