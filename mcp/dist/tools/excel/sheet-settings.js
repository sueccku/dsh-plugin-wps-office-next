...
exports.sheetSettingsTools = exports.getFormulaAuditHandler = exports.getFormulaAuditDefinition = exports.resetPageBreaksHandler = exports.resetPageBreaksDefinition = exports.setOutlineLevelsHandler = exports.setOutlineLevelsDefinition = exports.setSheetAppearanceHandler = exports.setSheetAppearanceDefinition = exports.setSheetHeaderFooterHandler = exports.setSheetHeaderFooterDefinition = exports.setSheetPrintTitlesHandler = exports.setSheetPrintTitlesDefinition = exports.setSheetPageSetupHandler = exports.setSheetPageSetupDefinition = exports.getSheetSettingsHandler = exports.getSheetSettingsDefinition = void 0;
/**
...
const PAPER_NAMES = { 1: 'Letter', 3: 'Tabloid', 5: 'Legal', 8: 'A3', 9: 'A4', 11: 'A5', 13: 'B5' };
/** 清掉标签色之后 Tab.Color 读回 0（黑），只有 ColorIndex = -4142 才说明「用的是默认色」 */
function describeTabColor(s) {
    if (s.tabColorIndex === -4142)
        return '(默认)';
    if (s.tabColor === undefined || s.tabColor === null)
        return '(默认)';
...
function describeSettings(s) {
    ...
    if (s.zoom === false)
        lines.push('  缩放: 按页适配（宽 ' + String(s.fitToPagesWide ?? '?') + ' 页 / 高 ' + String(s.fitToPagesTall ?? '?') + ' 页）');
    else
        lines.push('  缩放: ' + String(s.zoom ?? '?') + '%');
...
/** 读全部页面/打印/外观设置 */
exports.getSheetSettingsDefinition = {
...
const getSheetSettingsHandler = async (args) => {
    try {
        const response = await wps_client_1.wpsClient.executeMethod('getSheetSettings', { sheet: args.sheet }, wps_1.WpsAppType.SPREADSHEET);
        if (!response.success)
            return settingsFail('读取页面设置失败', response.error);
        return { id: (0, uuid_1.v4)(), success: true, content: [{ type: 'text', text: describeSettings(response.data || {}) }] };
    }
    catch (error) {
...
exports.getSheetSettingsHandler = getSheetSettingsHandler;
/** 页面设置 */
exports.setSheetPageSetupDefinition = {
...
const setSheetPageSetupHandler = async (args) => {
    try {
        const response = await wps_client_1.wpsClient.executeMethod('setSheetPageSetup', {
        ...
        if (!response.success)
            return settingsFail('设置页面失败', response.error);
...
exports.setSheetPageSetupHandler = setSheetPageSetupHandler;
/** 打印标题 */
exports.setSheetPrintTitlesDefinition = {
...
const setSheetPrintTitlesHandler = async (args) => {
    try {
        const response = await wps_client_1.wpsClient.executeMethod('setSheetPrintTitles', { sheet: args.sheet, printTitleRows: args.printTitleRows, printTitleColumns: args.printTitleColumns }, wps_1.WpsAppType.SPREADSHEET);
        if (!response.success)
            return settingsFail('设置打印标题失败', response.error);
...
exports.setSheetPrintTitlesHandler = setSheetPrintTitlesHandler;
/** 页眉页脚 */
exports.setSheetHeaderFooterDefinition = {
...
const setSheetHeaderFooterHandler = async (args) => {
    try {
        const response = await wps_client_1.wpsClient.executeMethod('setSheetHeaderFooter', {
        ...
        if (!response.success)
            return settingsFail('设置页眉页脚失败', response.error);
...
exports.setSheetHeaderFooterHandler = setSheetHeaderFooterHandler;
/** 工作表外观：隐藏与标签色 */
exports.setSheetAppearanceDefinition = {
...
const setSheetAppearanceHandler = async (args) => {
    try {
        const response = await wps_client_1.wpsClient.executeMethod('setSheetAppearance', { sheet: args.sheet, visible: args.visible, tabColor: args.tabColor }, wps_1.WpsAppType.SPREADSHEET);
        if (!response.success)
            return settingsFail('设置工作表外观失败', response.error);
...
exports.setSheetAppearanceHandler = setSheetAppearanceHandler;
/** 分级显示 */
exports.setOutlineLevelsDefinition = {
...
const setOutlineLevelsHandler = async (args) => {
    try {
        const response = await wps_client_1.wpsClient.executeMethod('setOutlineLevels', { sheet: args.sheet, rowLevels: args.rowLevels, columnLevels: args.columnLevels, summaryRow: args.summaryRow, summaryColumn: args.summaryColumn }, wps_1.WpsAppType.SPREADSHEET);
        if (!response.success)
            return settingsFail('设置分级显示失败', response.error);
        const d = response.data || {};
        const applied = d.applied || [];
        const head = applied.length ? '分级显示已更新（' + applied.join(', ') + '）' : '分级显示未变化';
        const where = (d.summaryRow === 0 ? '上' : '下') + ' / ' + (d.summaryColumn === -1 ? '左' : '右');
        return { id: (0, uuid_1.v4)(), success: true, content: [{ type: 'text', text: head + '；汇总行/列位置: ' + where }] };
    }
    catch (error) {
        const errMsg = error instanceof Error ? error.message : String(error);
...
exports.setOutlineLevelsHandler = setOutlineLevelsHandler;
/** 清除分页符 */
exports.resetPageBreaksDefinition = {
    name: 'wps_excel_reset_page_breaks',
    description: '清除工作表上的手动分页符，恢复按内容自动分页。使用场景：手工插过分页符之后想回到自动分页。',
    category: tools_1.ToolCategory.SPREADSHEET,
    inputSchema: { type: 'object', properties: { sheet: sheetParam } },
};
const resetPageBreaksHandler = async (args) => {
    try {
...
exports.resetPageBreaksHandler = resetPageBreaksHandler;
/** 公式审计：引用追踪 */
exports.getFormulaAuditDefinition = {
...
const getFormulaAuditHandler = async (args) => {
    try {
        const response = await wps_client_1.wpsClient.executeMethod('getFormulaAudit', { sheet: args.sheet, cell: args.cell, showPrecedents: args.showPrecedents, showDependents: args.showDependents, clearArrows: args.clearArrows }, wps_1.WpsAppType.SPREADSHEET);
        if (!response.success)
            return settingsFail('公式审计失败', response.error);
        const d = response.data || {};
...
exports.getFormulaAuditHandler = getFormulaAuditHandler;
exports.sheetSettingsTools = [
    { definition: exports.getSheetSettingsDefinition, handler: exports.getSheetSettingsHandler },
    { definition: exports.setSheetPageSetupDefinition, handler: exports.setSheetPageSetupHandler },
    { definition: exports.setSheetPrintTitlesDefinition, handler: exports.setSheetPrintTitlesHandler },
    { definition: exports.setSheetHeaderFooterDefinition, handler: exports.setSheetHeaderFooterHandler },
    { definition: exports.setSheetAppearanceDefinition, handler: exports.setSheetAppearanceHandler },
    { definition: exports.setOutlineLevelsDefinition, handler: exports.setOutlineLevelsHandler },
    { definition: exports.resetPageBreaksDefinition, handler: exports.resetPageBreaksHandler },
    { definition: exports.getFormulaAuditDefinition, handler: exports.getFormulaAuditHandler },
];
exports.default = exports.sheetSettingsTools;
//# sourceMappingURL=sheet-settings.js.map