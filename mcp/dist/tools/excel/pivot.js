...
exports.pivotTools = exports.updatePivotTableHandler = exports.updatePivotTableDefinition = exports.createPivotTableHandler = exports.createPivotTableDefinition = void 0;
 ...
 */
exports.createPivotTableDefinition = {
 ...
 * 创建透视表的Handler
 */
const createPivotTableHandler = async (args) => {
    const config = args;
    // 参数校验：必填项检查
    ...
    // 校验聚合类型
    const validAggregations = ['SUM', 'COUNT', 'AVERAGE', 'MAX', 'MIN'];
    ...
    try {
        const response = await wps_client_1.wpsClient.executeMethod('createPivotTable', {
...
exports.createPivotTableHandler = createPivotTableHandler;
/**
 * 更新透视表配置的Tool定义
 ...
 * 更新透视表配置的Handler
 */
const updatePivotTableHandler = async (args) => {
    const { pivotTableName, pivotTableCell, addRowFields, removeRowFields, addColumnFields, removeColumnFields, addValueFields, removeValueFields, updateValueFields, addFilterFields, removeFilterFields, refresh, } = args;
    // 至少要有一种方式定位透视表
    ...
    // 检查是否有实际的操作
    const hasOperations = (addRowFields && addRowFields.length > 0) ||
    ...
    // 校验值字段的聚合类型
    const validAggregations = ['SUM', 'COUNT', 'AVERAGE', 'MAX', 'MIN'];
    const validateValueFields = (fields, fieldName) => {
        if (!fields)
            return null;
    ...
    if (addValueFieldsError)
        return addValueFieldsError;
    const updateValueFieldsError = validateValueFields(updateValueFields, 'updateValueFields');
    if (updateValueFieldsError)
        return updateValueFieldsError;
    try {
        const response = await wps_client_1.wpsClient.executeMethod('updatePivotTable', {
...
exports.updatePivotTableHandler = updatePivotTableHandler;
/**
 * 导出所有透视表相关的Tools
 * 整整齐齐，方便注册
 */
exports.pivotTools = [
    { definition: exports.createPivotTableDefinition, handler: exports.createPivotTableHandler },
    { definition: exports.updatePivotTableDefinition, handler: exports.updatePivotTableHandler },
];
exports.default = exports.pivotTools;
