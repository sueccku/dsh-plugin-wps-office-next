...
export const getOpenWorkbooksHandler: ToolHandler = async (
  ...
  try {
    const response = await wpsClient.executeMethod<{
      workbooks: Array<string | { name?: string; path?: string; sheets?: number; active?: boolean }>;
    }>(
      'getOpenWorkbooks',
      {},
      WpsAppType.SPREADSHEET
    ...
    const list = (response.data?.workbooks || []).map((entry) => {
      ...
      if (entry?.path && entry.path !== entry.name) parts.push(entry.path);
      if (typeof entry?.sheets === 'number') parts.push(`${entry.sheets} 个工作表`);
      if (entry?.active) parts.push('当前活动');
      return parts.join(' | ');
    });
...
export const clearRangeHandler: ToolHandler = async (
  ...
  try {
    ...
    return { id: uuidv4(), success: true, content: [{ type: 'text', text: `范围 ${range} 的${typeLabel}已清除${impactText(response.data?.impact)}` }] };
  } catch (error) {
    const errMsg = error instanceof Error ? error.message : String(error);
