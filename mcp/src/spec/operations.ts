import { op, OperationSpec } from './types';
...
export const operations: OperationSpec[] = [
  ...
  op({
    ...
    "action": "setAnimation",
    "app": "ppt",
    "summary": "给形状加动画，三种用法：给 preset 就按预设给整页（或指定形状）加依次出场的动画；给 effect 且 effectKind=emphasis 加强调动画；只给 effect 则加入场/退场动画。使用场景：让要点逐个出现、强调某个数字。",
    "params": {
      "preset": {
        "type": "string",
        "description": "整页预设动画；给了它就忽略 effect",
        "enum": [
          "fadeIn",
          "flyIn",
          "zoomIn",
      ...
      "effect": {
        "type": "string",
        "description": "效果名：入场/退场用 fadeIn/flyIn/wipeIn/zoomIn/bounceIn/spinIn/fadeOut/flyOut；强调用 pulse/spin/grow/teeter"
      },
      "effectKind": {
        "type": "string",
        "description": "效果类别，默认 entrance",
        "enum": [
          "entrance",
          "emphasis"
        ]
      ...
      "duration": {
        "type": "number",
        "description": "强调动画时长（秒），默认 0.5"
      },
      "delayIncrement": {
        "type": "number",
        "description": "预设动画里每个形状之间的延迟增量（秒），默认 0.3"
      },
      "slideIndex": {
        "type": "number",
      ...
      "shapeIndex": {
        "type": "number",
        "description": "形状序号（从 1 开始）"
      },
      "shapeName": {
        "type": "string",
        "description": "形状名称；给了它就用名称定位"
      },
      "presentationName": {
        "type": "string",
  ...
  op({
    "tool": "wps_ppt_set_slide_footer",
    "action": "setSlideFooter",
    "app": "ppt",
    "summary": "一次设置演示文稿的页脚三件套：幻灯片编号、页脚文字、日期时间，以及日期的显示格式。只改给出来的项。使用场景：交付前统一加上页码与「内部资料」。",
    "params": {
      "showSlideNumber": {
        "type": "boolean",
        "description": "是否显示幻灯片编号"
      },
      "footerText": {
        "type": "string",
        "description": "页脚文字"
      ...
      "showFooter": {
        "type": "boolean",
        "description": "是否显示页脚；给了 footerText 而不给它就默认显示"
      },
      "showDate": {
        "type": "boolean",
        "description": "是否显示日期"
      ...
      "dateFormat": {
        "type": "string",
        "description": "日期显示格式，如 YYYY-MM-DD（按当前日期渲染成固定文本）"
      },
      "autoUpdate": {
        "type": "boolean",
        "description": "日期是否由程序自动刷新；false 表示钉住当前日期"
      },
      "presentationName": {
        "type": "string",
        "description": "演示文稿名；不填用当前文稿"
  ...
  op({
    "tool": "wps_ppt_set_table_format",
    "action": "setPptTableFormat",
    "app": "ppt",
    "summary": "设置表格的外观与样式。位置尺寸：left/top/width/height。样式作用域：给 row 和 col 就只改那一格，只给 row 改一整行，都不给但有样式键就改整张表；样式键为 backgroundColor/fontColor/fontSize/bold。使用场景：给表头行加底色、把表格挪到中间。",
    "params": {
      "left": {
        "type": "number",
        "description": "表格左边缘位置"
      ...
      "row": {
        "type": "number",
        "description": "第几行（从 1 开始）；与 col 一起用就是单元格"
      },
      "col": {
        "type": "number",
      ...
      "backgroundColor": {
        "type": "string",
        "description": "底纹颜色，十六进制如 #D9E2F3"
      },
      "fontColor": {
        "type": "string",
        "description": "文字颜色，十六进制如 #1A365D"
      },
      "fontSize": {
        "type": "number",
        "description": "字号"
      ...
      "tableIndex": {
        "type": "number",
        "description": "该页第几张表（从 1 开始），默认 1"
      },
      "tableName": {
        "type": "string",
        "description": "表格形状名；给了它就用名称定位"
      },
      "presentationName": {
        "type": "string",
        "description": "演示文稿名；不填用当前文稿"
