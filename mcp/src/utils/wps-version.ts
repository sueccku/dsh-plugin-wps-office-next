...
export const MIN_WPS_MAJOR = 12;
export const MIN_WPS_MINOR = 1;
 ...
 */
export function parseWpsVersion(raw: unknown): number[] {
...
export function wpsVersionTooOld(version: number[]): boolean {
  if (version.length < 2) return false;
  const [major, minor] = version;
  return major < MIN_WPS_MAJOR || (major === MIN_WPS_MAJOR && minor < MIN_WPS_MINOR);
}

/** 版本过旧时给出面向用户的一句中文提醒；版本读不到或够新时返回 undefined。 */
export function wpsCompatibilityWarning(appName: string | undefined, rawVersion: unknown): string | undefined {
  const version = parseWpsVersion(rawVersion);
  if (!wpsVersionTooOld(version)) return undefined;
  return (
    `检测到 ${appName || 'WPS'} 版本 ${String(rawVersion)}，低于本插件支持的 ${MIN_WPS_MAJOR}.${MIN_WPS_MINOR}；` +
